...
if __name__ == "__main__":
	...
	for i in range(1,epoch+1):
		...
		error = error + L1_coeff*np.sign(np.average(W + b))			#For L1 regularization
		#error = error + L2_coeff*np.square(np.average(W + b))		#For L2 regularization
		...
		W = W - lr*gradient(W, b, T_train, X_train_standard) + L1_coeff*np.sign(W)		#For L1 regularization
		#print(W)
		b = b - lr*grad_bias + L1_coeff*np.sign(b)

		# W = W - lr*gradient(W, b, T_train, X_train_standard) + L2_coeff*2*(W)			#For L2 regularization
